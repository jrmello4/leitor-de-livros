...
9. Extrair CSS mobile/webtoon de `styles.css` e reduzir `App.tsx`/`ReaderView.tsx`. (Feito: split em 6 módulos byte-idêntico; `ReaderView` em lazy — inicial 458→349KB; `App.tsx` 1718→539 linhas com 6 hooks: `useLibraryMetadata`, `useReadingProfiles`, `useNativeLibraryBoot`, `usePublicationActions`, `useReadingSession`, `useLibraryImport`.)
